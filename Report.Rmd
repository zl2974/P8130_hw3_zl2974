...
$$CI = (\bar{X_1}-\bar{X_2}~-~z_{1-\alpha/2}s_{pool}/\sqrt{1/n_1+1/n_0},
~\bar{X_1}-\bar{X_2}~-~z_{1-\alpha/2}s_{pool}/\sqrt{1/n_1+1/n_0} )$$
...
3) The testing in 1),2) are based on that sample is draw from a normal distributed population or the distributions fit normality. And thus, if we used these test method upon a skewed(not normal) sample, the result of our interpretation can be wrong. As plotted below, the sample distribution of within pairs difference, although observing a bipeak distribution in the intervention groups, using qq plot help us conclude that the point is center around qq line, satisfied normality requirement for testing.
